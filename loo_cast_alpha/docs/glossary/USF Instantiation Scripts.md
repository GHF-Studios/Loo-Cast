...
singleton-like [[Concept Declaration Artifact]]s for contract-defined USF concept kinds from the [[USF Contract]].
These declarations target Rust-registered [[Concept Archetype]] template authorities.
Executing these declaration entrypoints with profile-tailored `ctx` capability-object subgraphs emits full
declaration artifacts that include structured data plus logic closures.
...
Runtime activation then materializes active USF concept instances from those frozen declaration artifacts into the
[[USF Instance Graph]].
