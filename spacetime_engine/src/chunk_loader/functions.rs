...
fn calculate_spawn_priority(distance_squared: i32, radius_squared: i32) -> ChunkActionPriority {
    ...
    } else if normalized_distance <= 0.75 {
...
fn calculate_despawn_priority(distance_squared: i32, radius_squared: i32) -> ChunkActionPriority {
    let normalized_distance = distance_squared as f32 / radius_squared as f32;
...
pub(in crate) fn load_chunk(
    ...
    if !is_loaded {
        if !is_spawning && !is_despawning && !is_transfering_ownership {
            ...
            let priority = calculate_spawn_priority(distance_squared, radius_squared);
...
pub(in crate) fn unload_chunk(
    ...
    if loaded && !despawning {
        ...
        match chunk_loader_query.iter().find(|(loader_entity, transform, loader)| {
            ...
            is_chunk_in_loader_range(&chunk_coord, transform.translation.truncate(), loader.radius)
        }) {
            Some((new_owner, _, _)) => {
                chunk_action_buffer.add_action(ChunkAction::TransferOwnership {
                    coord: chunk_coord,
            ...
            None => {
                ...
                let priority = calculate_despawn_priority(distance_squared, radius_squared);
