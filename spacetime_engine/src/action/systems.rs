...
use super::{
    ...
    resources::{ActionMap, ActionTargetTypeRegistry},
    stage::{ActionStage, ActionStageEcs},
    stage_io::ActionStageIO,
    types::ActionState,
...
fn finalize_completed_actions(
    ...
    for (target_type, entity) in completed_actions {
        if let Some((instances, entity_index)) = action_map.map.get_mut(&target_type) {
...
fn collect_action_data(world: &mut World) -> (Vec<(Entity, String, String, usize, ActionStage, Box<dyn Any + Send + Sync>)>, Sender<ActionStageProcessedEvent>) {
    let mut system_state: SystemState<(ResMut<ActionMap>, ResMut<ActionTargetTypeRegistry>, Res<ActionStageProcessedMessageSender>)> = SystemState::new(world);
    let (mut action_map, mut target_type_registry, async_sender) = system_state.get_mut(world);
...
fn process_actions(
    ...
    for (entity, target_type, action_name, current_stage, mut stage, data_buffer) in actions_to_process.drain(..) {
