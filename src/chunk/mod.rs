...
impl ChunkID {
    pub fn from_base10_global(global_id_base10: BigUint) -> Result<ChunkID, String> {
        let global_id_base10x10 = BASE10X10_CONVERTER.convert_to_base10x10(global_id_base10.clone()).map_err(|e| format!("Computing the Base10x10 ID failed: {}", e))?;
        let global_id_base57 = BASE57_CONVERTER.convert_to_base57(global_id_base10.clone()).map_err(|e| format!("Computing the Base57 ID failed: {}", e))?;
        
        let mut chunk_id = ChunkID {
            scale_level: global_id_base10x10.len() as u8,
            local_id: (global_id_base10 % BigUint::from(100u32)).to_u8().unwrap(),
            cluster_id: None,
    ...
    pub fn from_base10x10_global(global_id_base10x10: Vec<(u8, u8)>) -> Result<ChunkID, String> {
        let global_id_base10 = BASE10X10_CONVERTER.convert_from_base10x10(global_id_base10x10.clone()).map_err(|e| format!("Computing the Base10 ID failed: {}", e))?;
        let global_id_base57 = BASE57_CONVERTER.convert_to_base57(global_id_base10.clone()).map_err(|e| format!("Computing the Base57 ID failed: {}", e))?;
    ...
    pub fn from_base57_global(global_id_base57: &str) -> Result<ChunkID, String> {
        let global_id_base10 = BASE57_CONVERTER.convert_from_base57(global_id_base57.clone()).map_err(|e| format!("Computing the Base10 ID failed: {}", e))?;
        let global_id_base10x10 = BASE10X10_CONVERTER.convert_to_base10x10(global_id_base10.clone()).map_err(|e| format!("Computing the Base10x10 ID failed: {}", e))?;
...
impl ChunkClusterID {
    pub fn from_chunk_id(chunk_id: ChunkID) -> ChunkClusterID {
        let base10_id = chunk_id.get_base10() / BigUint::from(100u32);
        let base57_id = BASE57_CONVERTER.convert_to_base57(base10_id.clone()).unwrap();
