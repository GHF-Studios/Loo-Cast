    ...
    pub fn generate(&self, state_type_name: String, output_type_name: String, _error_type_name: String, dyn_error_type_name: String) -> TokenStream {
        ...
        match self.signature.function_type {
            CoreFunctionType::RunEcs { .. } => match (has_input, has_output, has_error) {
                ...
                (false, false, true) => {
                    quote! {
                        fn run_ecs(main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorEVariant + Send + Sync> });
                ...
                (false, true, false) => {
                    quote! {
                        fn run_ecs(main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(output, #output_type_name.to_string()))
                        }
                ...
                (false, true, true) => {
                    quote! {
                        fn run_ecs(main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_ecs").entered();
                            let result = run_ecs_inner(main_access);
                            let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorOEVariant + Send + Sync> });
                ...
                (true, false, false) => {
                    quote! {
                        fn run_ecs(input: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_ecs").entered();
                ...
                (true, false, true) => {
                    quote! {
                        fn run_ecs(input: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorIEVariant + Send + Sync> });
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(result, format!("Result<(), {}>", #dyn_error_type_name.to_string()).to_string()))
                        }
                ...
                (true, true, true) => {
                    quote! {
                        fn run_ecs(input: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            let input = input.unwrap().into_inner::<Input>();
                            let result = run_ecs_inner(input, main_access);
                            let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorIOEVariant + Send + Sync> });
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(result, format!("Result<{}, {}>", #output_type_name.to_string(), #dyn_error_type_name.to_string()).to_string()))
                        }
            ...
            CoreFunctionType::RunRender { .. } => match (has_input, has_output, has_error) {
                ...
                (true, false, false) => {
                    quote! {
                        fn run_render(input: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, render_access: RenderAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_render").entered();
                            let input = input.unwrap().into_inner::<Input>();
            ...
            CoreFunctionType::RunAsync { .. } => match (has_input, has_output, has_error) {
                ...
                (true, false, false) => {
                    quote! {
                        fn run_async(input: Option<crate::utils::premium_box::AnySendSyncPremiumBox>) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_async").entered();
                            let input = input.unwrap().into_inner::<Input>();
            ...
            CoreFunctionType::SetupEcsWhile { .. } => match (has_input, has_state, has_error) {
                ...
                (false, false, true) => {
                    let result_mapper = if has_output {
                        quote! { let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorOEVariant + Send + Sync> }); }
                    } else {
                        quote! { let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorEVariant + Send + Sync> }); }
                    };
                    quote! {
                        fn setup_ecs_while(main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("setup_ecs_while").entered();
                ...
                (false, true, false) => {
                    quote! {
                        fn setup_ecs_while(main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(state, #state_type_name.to_string()))
                        }
            ...
            CoreFunctionType::RunEcsWhile { .. } => match (has_state, has_output, has_error) {
                (false, false, false) => {
                    quote! {
                        fn run_ecs_while(_state: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(outcome, format!("Outcome<(), ()>").to_string()))
                        }
                ...
                (false, false, true) => {
                    let result_mapper = if has_input {
                        quote! { let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorIEVariant + Send + Sync> }); }
                    } else {
                        quote! { let result = result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorEVariant + Send + Sync> }); }
                    };
                    quote! {
                ...
                (false, true, false) => {
                    quote! {
                        fn run_ecs_while(_state: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_ecs_while").entered();
                            let outcome = run_ecs_while_inner(main_access);
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(outcome, format!("Outcome<(), {}>", #output_type_name.to_string()).to_string()))
                        }
                ...
                (false, true, true) => {
                    let result_mapper = if has_input {
                        quote! { outcome_result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorIOEVariant + Send + Sync> }); }
                    } else {
                        quote! { outcome_result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorOEVariant + Send + Sync> }); }
                    };
                    quote! {
                        fn run_ecs_while(_state: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_ecs_while").entered();
                ...
                (true, false, false) => {
                    quote! {
                        fn run_ecs_while(state: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(outcome, format!("Outcome<{}, ()>", #state_type_name.to_string()).to_string()))
                        }
                ...
                (true, false, true) => {
                    let result_mapper = if has_input {
                        quote! { outcome_result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorIEVariant + Send + Sync> }); }
                    } else {
                        quote! { outcome_result.map_err(|e| { Box::new(e) as Box<dyn crate::workflow::traits::WorkflowErrorEVariant + Send + Sync> }); }
                    };
                    quote! {
                        fn run_ecs_while(state: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            let _span = info_span!("run_ecs_while").entered();
                ...
                (true, true, false) => {
                    quote! {
                        fn run_ecs_while(state: Option<crate::utils::premium_box::AnySendSyncPremiumBox>, main_access: MainAccess) -> Option<crate::utils::premium_box::AnySendSyncPremiumBox> {
                            ...
                            let state = state.unwrap().into_inner::<State>();
                            let outcome = run_ecs_while_inner(state, main_access);
                            Some(crate::utils::premium_box::AnySendSyncPremiumBox::new(outcome, format!("Outcome<{}, {}>", #state_type_name.to_string(), #output_type_name.to_string()).to_string()))
                        }
                    ...
                    (false, false, false) => {
                        quote! {
                            pub fn poll_ecs_stage_buffer_system(mut stage_buffer: bevy::prelude::ResMut<StageBuffer>, main_access: MainAccess) {
                                ...
                                let handle = crate::workflow::statics::WORKFLOW_TOKIO_RUNTIME()
                                    .handle()
                                    .clone();
