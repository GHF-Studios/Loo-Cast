...
impl Parse for TypedWorkflowPath {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        if input.is_empty() {
            return Ok(TypedWorkflowPath::None(WorkflowPath { workflow_module_name, workflow_name }))
        }

        let lookahead = input.lookahead1();
        if lookahead.peek(kw::E) {
            input.parse::<kw::E>()?;
            Ok(TypedWorkflowPath::E(WorkflowPath { workflow_module_name, workflow_name }))
        } else if lookahead.peek(kw::O) {
            input.parse::<kw::O>()?;
            Ok(TypedWorkflowPath::O(WorkflowPath { workflow_module_name, workflow_name }))
        } else if lookahead.peek(kw::OE) {
            input.parse::<kw::OE>()?;
            Ok(TypedWorkflowPath::OE(WorkflowPath { workflow_module_name, workflow_name }))
        } else if lookahead.peek(kw::I) {
            input.parse::<kw::I>()?;
            Ok(TypedWorkflowPath::I(WorkflowPath { workflow_module_name, workflow_name }))
        } else if lookahead.peek(kw::IE) {
            input.parse::<kw::IE>()?;
            Ok(TypedWorkflowPath::IE(WorkflowPath { workflow_module_name, workflow_name }))
        } else if lookahead.peek(kw::IO) {
            input.parse::<kw::IO>()?;
            Ok(TypedWorkflowPath::IO(WorkflowPath { workflow_module_name, workflow_name }))
        } else if lookahead.peek(kw::IOE) {
            input.parse::<kw::IOE>()?;
            Ok(TypedWorkflowPath::IOE(WorkflowPath { workflow_module_name, workflow_name }))
        } else {
            Err(lookahead.error())
        }
    }
...
impl TypedWorkflowPath {
    fn convert_to_raw(path: WorkflowPath) -> (Ident, Ident) {
        let WorkflowPath { workflow_module_name: workflow_module_ident, workflow_name: workflow_ident } = path;
