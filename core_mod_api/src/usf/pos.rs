...
impl GridOffset {
    ...
    /// * `scale_origin` - The scale of this GridOffset
    /// # Panics
    /// * If the parent scale is not greater than the child scale
    /// * If the scale is not MAX and parent is None
    /// # Returns
    /// A new GridOffset at the given `scale_origin`'s true `grid_origin` at (0,0)
    pub fn new_origin(parent: Option<Box<GridOffset>>, scale_origin: Scale) -> Self {
        let parent = match parent {
            Some(parent) => {
                assert!(parent.scale_origin > scale_origin, "The parent's scale must be greater than child's scale");
                Some(parent)
            },
            None => {
                assert!(scale_origin == Scale::MAX, "Root GridPos must have MAX scale");
                None
            }
        };

        Self { parent, scale_origin, xy: I128Vec2::ZERO }
    }
    ...
    /// # Returns
    /// A new GridOffset at the given `xy`, from the given `scale_origin`'s true `grid_origin` at (0,0)
    pub fn new(parent: Option<Box<GridOffset>>, scale_origin: Scale, xy: I128Vec2) -> Self {
        let parent = match parent {
            Some(parent) => {
                assert!(parent.scale_origin > scale_origin, "Parent scale must be greater than child scale");
                Some(parent)
            },
            None => {
                assert!(scale_origin == Scale::MAX, "Root GridPos must have MAX scale");
        ...
        let scale_extent = GRID_SIZE * 10_i128.pow(scale_origin.index_from_top() as u32);
        let half_scale_extent = scale_extent / 2;

        if xy.x < -half_scale_extent { panic!("X coordinate {} is too small for scale {:?}. Range is -{} to {}", xy.x, scale_origin, half_scale_extent, half_scale_extent); }
        if xy.x > half_scale_extent { panic!("X coordinate {} is too large for scale {:?}. Range is -{} to {}", xy.x, scale_origin, half_scale_extent, half_scale_extent); }
        if xy.y < -half_scale_extent { panic!("Y coordinate {} is too small for scale {:?}. Range is -{} to {}", xy.y, scale_origin, half_scale_extent, half_scale_extent); }
        if xy.y > half_scale_extent { panic!("Y coordinate {} is too large for scale {:?}. Range is -{} to {}", xy.y, scale_origin, half_scale_extent, half_scale_extent); }

        Self { parent, scale_origin, xy }
    }
}
...
impl GridPos {
    ...
    /// # Arguments
    /// * `scale_origin` - The currently active scale that this position is relative to
    /// * `grid_offset` - The grid offset from the `scale_origin`'s true `grid_origin` at (0,0)
    /// # Returns
    /// A new GridPos, at the given `grid_offset`, from the given `scale_origin`'s true `grid_origin` at (0,0)
    ...
    /// # Returns
    /// A new GridPos, at the given `subgrid_offset`, from the given `grid_offset`, from the given `scale_origin`'s true `grid_origin` at (0,0)
    pub fn new(scale_origin: Scale, grid_offset: GridOffset, subgrid_offset: Vec2) -> Self {
        let diff_scale_factor = grid_offset.scale_origin.difference_scale_factor(&scale_origin);
        ...
        let grid_offset = GridOffset::new(grid_offset.parent, grid_offset.scale_origin, grid_offset_xy);
...
impl SubgridPos {
    /// Create a new SubgridPos from a scale origin, and a grid offset
    /// # Arguments
    /// * `scale_origin` - The currently active scale that this position is relative to
    /// * `grid_offset` - The grid offset from the `scale_origin`'s true `grid_origin` at (0,0)
    /// # Returns
    /// A new SubgridPos, at the origin (0,0,0) of the given `grid_offset`, from the given `scale_origin`'s true `grid_origin` at (0,0)
    pub fn new_origin(scale_origin: Scale, grid_offset: GridOffset) -> Self {
    ...
    /// Create a new SubgridPos from a scale origin, a grid offset, and a subgrid offset
