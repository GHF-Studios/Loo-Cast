...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ChunkRealizationRecord {
...
pub(crate) fn generate_chunk_realization_record(
    world_seed: u64,
    sample_step: u16,
    iso_level: u8,
    chunk_scale: Scale,
...
pub(crate) fn load_chunk_realization_record(path: &Path) -> Option<ChunkRealizationRecord> {
    let bytes = fs::read(path).ok()?;
    serde_json::from_slice::<ChunkRealizationRecord>(&bytes).ok()
}

pub(crate) fn save_chunk_realization_record(path: &Path, record: &ChunkRealizationRecord) -> Result<(), String> {
...
mod tests {
    ...
    fn test_settings() -> TestSettings {
        TestSettings {
            ...
            sample_step: 64,
            iso_level: 128,
            persistence_dir: std::env::temp_dir().join("usf_runtime_realization_tests").to_string_lossy().to_string(),
        }
    }

    fn density_values(record: &ChunkRealizationRecord) -> Vec<u8> {
    ...
    fn rho_sampling_matches_across_adjacent_chunk_borders() {
        ...
        let left_record = generate_chunk_realization_record(
            settings.world_seed,
    ...
    fn persistence_roundtrip_is_stable() {
        ...
        let (zone_type, zone_density_profile) = test_zone();
        let record = generate_chunk_realization_record(
            settings.world_seed,
            settings.sample_step,
            settings.iso_level,
            Scale::MAX,
        ...
        save_chunk_realization_record(&path, &record).expect("save should succeed");
        let loaded = load_chunk_realization_record(&path).expect("load should succeed");
