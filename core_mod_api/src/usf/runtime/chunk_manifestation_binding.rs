...
#[reflect(Component)]
pub struct ChunkManifestationAuthorityGrace {
...
impl BindingSyncProbe {
    fn observe(&mut self, full_sync: bool, reason_flags: BindingSyncReasonFlags, counters: BindingSyncCounters) {
        ...
        warn!(
            "USF hotpath probe [manifestation_binding]: calls={}, full_sync_calls={}, dirty_only_calls={}, reasons(settings={}, execution_plan={}, substrate={}, definitions={}, zone_runtime={}, zone_realization={}, zone_behavior={}), scanned_chunks={}, upserted={}, removed={}, unchanged={}, none_to_none={}",
            self.calls,
...
pub(crate) fn sync_chunk_manifestation_bindings_system(
    ...
    chunk_query: Query<(Entity, &Chunk, Option<&ChunkManifestationBinding>, Option<&ChunkManifestationAuthorityGrace>)>,
    ...
    if full_sync {
        for (entity, chunk, existing_binding, existing_grace) in chunk_query.iter() {
            let action = sync_chunk_manifestation_binding_for_entity(
                entity,
                chunk,
...
fn sync_chunk_manifestation_binding_for_entity(
    entity: Entity,
    chunk: &Chunk,
    existing_binding: Option<ChunkManifestationBinding>,
    existing_grace: Option<ChunkManifestationAuthorityGrace>,
    manifestation_runtime_enabled: bool,
    ...
    if !manifestation_runtime_enabled {
        if existing_binding.is_some() || existing_grace.is_some() {
            commands.entity(entity).remove::<ChunkManifestationBinding>();
            commands.entity(entity).remove::<ChunkManifestationAuthorityGrace>();
    ...
    let desired = if manifestation_runtime_enabled {
        desired_chunk_manifestation_binding(
            chunk,
            execution_plan,
            substrate_store,
            phenomenon_definitions,
    ...
    match (existing_binding, desired) {
        ...
        (_, Some(next)) => {
            // Binding changes invalidate the current visual artifact and force deterministic rebuild.
        ...
        (Some(_), None) => {
            ...
            if next_missing_frames < grace_limit {
                commands.entity(entity).insert(ChunkManifestationAuthorityGrace {
...
fn desired_chunk_manifestation_binding(
    chunk: &Chunk,
    execution_plan: &UsfExecutionPlan,
    substrate_store: &AdaptiveSubstrateStore,
    phenomenon_definitions: &PhenomenonDefinitionRegistry,
    ...
    zone_realization_state: &ZoneRealizationState,
    zone_behavior_registry: &ZoneBehaviorRegistry,
) -> Option<ChunkManifestationBinding> {
    ...
    if zone_id.zone_type != zone_type {
        panic!(
            "USF manifestation binding failed: chunk {:?} classified as zone '{}' in substrate summary, but zone runtime mapped it to '{}'.",
    ...
    let zone_density_profile = zone_behavior_registry
        ...
        .unwrap_or_else(|| panic!("USF manifestation binding failed: missing zone density profile for zone '{}'.", zone_type.0));
    ...
    if !supports_zone_phenomenon {
        panic!(
            "USF manifestation binding failed: realized phenomenon '{}' is not declared in zone support policy for zone '{}'.",
            phenomenon_script_id, zone_type.0
        );
    }
    let Some(manifestation_field_contract) = phenomenon_definitions.manifestation_field_contract_for_scale(phenomenon_script_id.as_str(), chunk_scale) else {
        return None;
    };
...
mod tests {
    ...
    #[test]
    fn zone_realization_is_manifestation_binding_authority() {
