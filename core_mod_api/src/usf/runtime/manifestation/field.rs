...
use crate::usf::phenomenon::{ManifestationDensityFieldDefinition, PhenomenonManifestationFieldContract};
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct CachedChunkManifestationRecord {
    pub schema_version: u16,
    pub cache_authority: String,
    pub world_seed: u64,
    ...
    pub density_cache: MixedMetricFieldU8,
    pub zone_cache: MixedMetricFieldU32,
}

pub(crate) fn generate_chunk_manifestation_cache_record(
    world_seed: u64,
    sample_step: u16,
    iso_level: u8,
    chunk_scale: Scale,
    ...
    manifestation_field_contract: PhenomenonManifestationFieldContract,
) -> CachedChunkManifestationRecord {
    ...
    CachedChunkManifestationRecord {
        ...
        density_field_signature: density_field_signature(manifestation_field_contract),
        phenomenon_script_id: phenomenon_script_id.to_ascii_lowercase(),
        chunk_seed,
        sample_step,
        iso_level,
        axis_samples,
        density_cache: MixedMetricFieldU8::from_values(rho_values),
        zone_cache: MixedMetricFieldU32::from_values(zone_values),
    }
}
...
pub(crate) fn sample_density_field_values(
    ...
    {
        let _ = chunk_scale;
        sample_density_field_values_cpu(world_seed, canonical_coord, axis_samples, zone_density_profile, manifestation_field_contract)
    }

    #[cfg(not(test))]
    {
        let density_field = manifestation_density_field_from_contract(manifestation_field_contract);
        let semantics = gpu_density::GpuDensitySemantics {
            ...
            coarse_weight: density_field.coarse_weight,
            detail_weight: density_field.detail_weight,
            bias: density_field.bias,
            gain: density_field.gain,
            center: density_field.center,
            seed_salt_coarse: density_field.seed_salt_coarse,
            seed_salt_detail: density_field.seed_salt_detail,
            zone_density_multiplier: zone_density_profile.density_multiplier,
            zone_density_offset: zone_density_profile.density_offset,
            zone_density_floor: zone_density_profile.density_floor,
            zone_density_ceil: zone_density_profile.density_ceil,
        };
        match gpu_density::sample_density_field(world_seed, chunk_scale, canonical_coord, axis_samples, semantics) {
            Ok(values) => values,
            Err(error) => panic!(
                "USF runtime GPU density sampling failed (scale_index={}, coord={:?}): {}",
...
fn sample_density_field_values_cpu(
    ...
    let manifestation_density_field = manifestation_density_field_from_contract(manifestation_field_contract);
    let axis_points = axis_samples.len();
    let total_points = axis_points * axis_points * axis_points;
    ...
    for iz in 0..axis_points {
        for iy in 0..axis_points {
            for ix in 0..axis_points {
                ...
                rho_values.push(hash_density_u8(world_seed, root_native, zone_density_profile, manifestation_density_field));
            }
        }
    }
...
pub(crate) fn load_chunk_manifestation_cache_record(path: &Path) -> Option<CachedChunkManifestationRecord> {
    let bytes = fs::read(path).ok()?;
    serde_json::from_slice::<CachedChunkManifestationRecord>(&bytes).ok()
}

pub(crate) fn save_chunk_manifestation_cache_record(path: &Path, record: &CachedChunkManifestationRecord) -> Result<(), String> {
...
pub(crate) fn density_field_signature(manifestation_field_contract: PhenomenonManifestationFieldContract) -> u64 {
    let manifestation_density_field = manifestation_density_field_from_contract(manifestation_field_contract);
    ...
    signature_seed ^= manifestation_density_field.coarse_span_units.to_bits();
    signature_seed ^= manifestation_density_field.detail_span_units.to_bits();
    signature_seed ^= (manifestation_density_field.coarse_weight.to_bits() as u64) << 1;
    signature_seed ^= (manifestation_density_field.detail_weight.to_bits() as u64) << 2;
    signature_seed ^= (manifestation_density_field.bias.to_bits() as u64) << 3;
    signature_seed ^= (manifestation_density_field.gain.to_bits() as u64) << 4;
    signature_seed ^= (manifestation_density_field.center.to_bits() as u64) << 5;
    signature_seed ^= manifestation_density_field.seed_salt_coarse;
    signature_seed ^= manifestation_density_field.seed_salt_detail;
...
fn hash_density_u8(
    ...
    zone_density_profile: ZoneDensityProfile,
    manifestation_density_field: ManifestationDensityFieldDefinition,
) -> u8 {
    ...
    let coarse_seed = mix32(
        world_seed_folded ^ (manifestation_density_field.seed_salt_coarse & 0xffff_ffff) as u32 ^ (manifestation_density_field.seed_salt_coarse >> 32) as u32,
    );
    let detail_seed = mix32(
        world_seed_folded ^ (manifestation_density_field.seed_salt_detail & 0xffff_ffff) as u32 ^ (manifestation_density_field.seed_salt_detail >> 32) as u32,
    );

    let base = value_noise_3d(coarse_seed, wx, wy, wz, manifestation_density_field.coarse_span_units);
    let detail = value_noise_3d(detail_seed, wx, wy, wz, manifestation_density_field.detail_span_units);
    let weight_sum = (manifestation_density_field.coarse_weight + manifestation_density_field.detail_weight).max(f32::MIN_POSITIVE);
    let combined = ((base * manifestation_density_field.coarse_weight) + (detail * manifestation_density_field.detail_weight)) / weight_sum;
    let shaped = ((combined - manifestation_density_field.bias) * manifestation_density_field.gain + manifestation_density_field.center).clamp(0.0, 1.0);
...
#[inline]
fn manifestation_density_field_from_contract(manifestation_field_contract: PhenomenonManifestationFieldContract) -> ManifestationDensityFieldDefinition {
    match manifestation_field_contract {
        PhenomenonManifestationFieldContract::DensityField(field) => field,
    }
}
...
fn value_noise_3d(seed: u32, gx: f64, gy: f64, gz: f64, cell_size: f64) -> f32 {
    let cell_size = cell_size.max(f64::EPSILON);
    let sx = gx / cell_size;
    let sy = gy / cell_size;
    let sz = gz / cell_size;

    let cx0 = sx.floor() as i32;
    let cy0 = sy.floor() as i32;
    let cz0 = sz.floor() as i32;
    let cx1 = cx0 + 1;
    let cy1 = cy0 + 1;
    let cz1 = cz0 + 1;
...
fn lattice_noise01(seed: u32, x: i32, y: i32, z: i32) -> f32 {
    let mut state = mix32(seed ^ 0x5f35_d3a1_u32);
    state = mix32(state ^ x as u32);
    state = mix32(state ^ y as u32);
    state = mix32(state ^ z as u32);
...
mod tests {
    ...
    fn test_settings() -> TestSettings {
        TestSettings {
            ...
            sample_step: 64,
            iso_level: 128,
            persistence_dir: std::env::temp_dir().join("usf_runtime_manifestation_tests").to_string_lossy().to_string(),
        }
    }

    fn density_values(record: &CachedChunkManifestationRecord) -> Vec<u8> {
        let axis_points = record.axis_samples.len();
        let total = axis_points * axis_points * axis_points;
        record.density_cache.expand(total).expect("density cache should expand")
    }
    ...
    fn test_phenomenon_id() -> &'static str {
        "phenomenon.test.manifestation_density"
    }

    fn test_manifestation_density_field() -> ManifestationDensityFieldDefinition {
        ManifestationDensityFieldDefinition::default()
    }

    fn test_manifestation_field_contract() -> PhenomenonManifestationFieldContract {
        PhenomenonManifestationFieldContract::DensityField(test_manifestation_density_field())
    }
    ...
    fn rho_sampling_matches_across_adjacent_chunk_borders() {
        ...
        let left_record = generate_chunk_manifestation_cache_record(
            settings.world_seed,
            ...
            zone_density_signature,
            test_phenomenon_id(),
            test_manifestation_field_contract(),
        );
        let right_record = generate_chunk_manifestation_cache_record(
            settings.world_seed,
            settings.sample_step,
            settings.iso_level,
            Scale::MAX,
        ...
        let left_values = density_values(&left_record);
        let right_values = density_values(&right_record);
    ...
    fn persistence_roundtrip_is_stable() {
        ...
        let (zone_type, zone_density_profile) = test_zone();
        let record = generate_chunk_manifestation_cache_record(
            settings.world_seed,
            settings.sample_step,
            settings.iso_level,
            Scale::MAX,
        ...
        save_chunk_manifestation_cache_record(&path, &record).expect("save should succeed");
        let loaded = load_chunk_manifestation_cache_record(&path).expect("load should succeed");
    ...
    fn density_sampling_is_scale_invariant_for_shared_root_space_points() {
        ...
        for child_local in [-500_i32, -300, 0, 300, 500] {
            ...
            let child_density = hash_density_u8(settings.world_seed, child_root_native, zone_density_profile, test_manifestation_density_field());
            let parent_density = hash_density_u8(
                settings.world_seed,
                parent_root_native,
                zone_density_profile,
                test_manifestation_density_field(),
            );
            assert_eq!(
