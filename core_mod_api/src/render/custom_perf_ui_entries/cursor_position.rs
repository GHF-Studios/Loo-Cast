...
pub struct PerfUiCursorPosEntries {
    ...
    pub unit_pos_unprojected: PerfUiEntryCursorUnitPosUnprojected,
}
...
#[derive(Component, Debug, Clone, Reflect)]
#[require(PerfUiRoot)]
pub struct PerfUiEntryCursorUnitPosUnprojected {
    pub label: String,
    pub sort_key: i32,
}
impl Default for PerfUiEntryCursorUnitPosUnprojected {
    fn default() -> Self {
        Self {
            label: "(C)UnitPosUnproj.".to_string(),
            sort_key: next_sort_key(),
        }
    }
}
impl PerfUiEntry for PerfUiEntryCursorUnitPosUnprojected {
    type SystemParam = (
        Local<'static, Option<Vec2>>,
        Query<'static, 'static, (&'static Camera, &'static GlobalTransform), With<MainCamera>>,
        Query<'static, 'static, &'static Window, With<PrimaryWindow>>,
