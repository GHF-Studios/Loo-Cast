...
impl TypeConstDynMetadata for TypeMetadata {
    ...
    fn generic_definition_id(&self) -> CloneLazy<Option<ImmutableString>> { self.generic_definition_id.clone() }
    fn generic_param_names(&self) -> CloneLazy<Vec<ImmutableString>> { self.generic_param_names.clone() }
    fn generic_param_trait_bounds(&self) -> CloneLazy<Vec<Vec<TraitPath>>> { self.generic_param_trait_bounds.clone() }
    fn generic_instantiation_args(&self) -> CloneLazy<Vec<Vec<TypePath>>> { self.generic_instantiation_args.clone() }
}
