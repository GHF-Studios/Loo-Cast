...
pub(super) fn flush_span_message_buffer_system(mut log_registry: ResMut<LogRegistry>) {
    while let Some(span_path) = SPAN_EVENT_BUFFER().pop() {
        log_registry.insert_without_log(&span_path);
    }
}
pub(super) fn flush_log_message_buffer_system(mut log_registry: ResMut<LogRegistry>) {
