...
pub fn run_ecs(input: Input, main_access: MainAccess) -> Output {
    ...
    let chunk_loader_grid_position = position.to_grid_coord(grid_xy.0);
    ...
    for chunk_to_despawn in chunks_to_despawn {
        let chunk_loader_distance_squared = chunk_to_despawn.xy.distance_squared(&chunk_loader_grid_position).try_into().unwrap();
