...
use crate::script::ecs::system::commands::{
...
impl CommandsApi for Shared<Commands> {
    fn spawn_empty(&self, ctx: NativeCallContext, callback: FnPtr) -> Dynamic {
        ...
        let output = callback.call_within_context::<Dynamic>(&ctx, (shared_entity_commands.clone(),));

        drop(shared_entity_commands);
...
impl EntityCommandsApi for Shared<EntityCommands> {
    fn commands(&self, ctx: NativeCallContext, f: FnPtr) -> Dynamic {
        ...
        let output = f.call_within_context::<Dynamic>(&ctx, (shared_commands.clone(),));
