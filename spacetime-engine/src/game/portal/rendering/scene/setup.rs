...
pub fn setup_portals(
    ...
    let frame_meshes = create_frame_meshes(&mut meshes, config.size);

    spawn_frame(
        &mut commands,
        first,
        &frame_meshes,
        materials.add(config.first.frame_color),
        config.size,
    );
    ...
    spawn_frame(
        ...
        second,
        &frame_meshes,
        materials.add(config.second.frame_color),
        config.size,
    );
    ...
    let portal_surface_mesh = meshes.add(Rectangle::new(
        config.size.x + 2.0 * SURFACE_OVERSCAN,
        config.size.y + 2.0 * SURFACE_OVERSCAN,
    ));
