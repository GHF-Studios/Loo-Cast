...
pub(crate) fn destination_clip_plane(
    ...
    let camera_to_portal = destination.translation - camera.translation;
    ...
    let plane_point = destination.translation - normal_world * margin;

    let view_rotation = camera.rotation.inverse();

    let normal_view = (view_rotation * normal_world).normalize();

    let point_view = view_rotation * (plane_point - camera.translation);
