...
pub const PROJECTILE_GUN: PlaygroundItemId =
    PlaygroundItemId::new("projectile_gun");

pub struct ProjectileGunItemPlugin;

impl Plugin for ProjectileGunItemPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(PreStartup, register_item).add_systems(
            Update,
            use_projectile_gun.in_set(GameSet::Action),
        );
    }
}
...
fn use_projectile_gun(
    ...
    for request in uses.read() {
        if request.item != PROJECTILE_GUN
