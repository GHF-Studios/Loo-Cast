...
pub fn ray_box_distance(ray: AimRay, transform: &Transform, half_extents: Vec3) -> Option<f32> {
    ...
    for axis in 0..3 {
        ...
        let mut first = (min[axis] - origin_axis) * inverse_direction;

        let mut second = (max[axis] - origin_axis) * inverse_direction;
